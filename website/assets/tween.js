...
var container;
var camera, scene, renderer, particle, raycaster, mouse;
...
function init() {
    ...
    var img = new THREE.MeshBasicMaterial({ //CHANGED to MeshBasicMaterial
        map: THREE.ImageUtils.loadTexture('./images/movies.jpg')
    });
    img.map.needsUpdate = true; //ADDED

    // plane
    var plane = new THREE.Mesh(new THREE.PlaneGeometry(6000,4000), img);
    plane.position.y = 0
    plane.position.x = 0
    plane.overdraw = true;
    scene.add(plane);

    var img1 = new THREE.MeshBasicMaterial({ //CHANGED to MeshBasicMaterial
        map: THREE.ImageUtils.loadTexture('./images/movies.jpg')
    });
    img1.map.needsUpdate = true; //ADDED

    // plane
    var plane1 = new THREE.Mesh(new THREE.PlaneGeometry(6000,4000), img1);
    plane1.position.y = 0
    plane1.position.x = 6000
    ...
    // plane
    var plane2 = new THREE.Mesh(new THREE.PlaneGeometry(6000,4000), img2);
    plane2.position.y = 0
    plane2.position.x = -6000
...
function onDocumentMouseDown( event ) {
    ...
    if ((intersects.length > 0) && (intersects[0].name = "arrow")) {

        $('html, body').animate({ scrollTop: $(".page2").offset().top}, 500, 'linear');
